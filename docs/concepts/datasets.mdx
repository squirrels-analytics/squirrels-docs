...
```text
/analytics/{project_name}/v{project_version}/api/0
```
...
```text
GET  {api_prefix}/datasets/{dataset_name}/parameters
POST {api_prefix}/datasets/{dataset_name}/parameters
```
...
```text
GET  {api_prefix}/datasets/{dataset_name}
POST {api_prefix}/datasets/{dataset_name}
```
...
- `Authorization: Bearer <token>` - Authorization header for Bearer token (required if auth is enabled)
- `x-api-key: <api_key>` - Alternative to `Authorization` when `auth_strategy` is `managed`
...
- `Authorization` / `x-api-key` - Authorization
  - Note: `x-api-key` only applies when `auth_strategy` is `managed`
...
For more information on these endpoints, you can find the swagger documentation of any running Squirrels server at the path `/analytics/{project_name}/v{project_version}/docs`.
