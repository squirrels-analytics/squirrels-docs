...
| `--log-level {DEBUG,INFO,WARNING}` | Level of logging to use. Default is from SQRL_LOGGING__LEVEL environment variable or INFO. |
| `--log-format {text,json}` | Format of the log records. Default is from SQRL_LOGGING__FORMAT environment variable or text. |
| `--log-to-file` | Enable logging to file(s) in the "logs/" folder (or a custom folder) with rotation and retention policies. Default is false unless overridden by the SQRL_LOGGING__TO_FILE environment variable. |
