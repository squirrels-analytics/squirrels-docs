...
<Expandable title="arguments" defaultOpen>
  ...
  <ResponseField name="log_to_file" type="boolean" default={false}>
    Whether to enable logging to file(s) in the "logs/" folder (or a custom folder). Default is from SQRL_LOGGING__TO_FILE environment variable or false.
  </ResponseField>
...
### get_http_app_and_lifespan()

Method to retrieve the FastAPI sub-application and its lifespan context manager. This is useful for mounting the Squirrels project into an existing FastAPI application.

```python
def get_http_app_and_lifespan(
    self, *, no_cache: bool = False
) -> tuple[FastAPI, AsyncGeneratorContextManager]:
```

<Expandable title="arguments" defaultOpen>
  <ResponseField name="no_cache" type="boolean" default={false}>
    Whether to disable caching for the API server.
  </ResponseField>
</Expandable>

<ResponseField name="returns" type="tuple[FastAPI, AsyncGeneratorContextManager]">
  A tuple containing the FastAPI app object and the lifespan context manager.
</ResponseField>

### get_mount_path()

Method to get a standard mount path based on project name and version.
...
<Expandable title="arguments" defaultOpen>
  <ResponseField name="path_format" type="string" default="/analytics/{project_name}/v{project_version}">
